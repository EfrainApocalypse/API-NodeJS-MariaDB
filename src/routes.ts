...
export async function routes(server: FastifyTypedInstance) {
	server.get(
		...
		async () => {
			var [mensagens] = (await db.query(
				"SELECT * FROM `mensagens` ORDER BY `hora` DESC",
			)) as any[];
			return mensagens;
		},
	);
	...
	server.post(
		...
		async (request, reply) => {
			...
			await db.query(
				"INSERT INTO `mensagens` (`autor`, `valor`, `hora`) VALUES (?, ?, now());",
				[autor, valor],
			);
	...
	server.put(
		...
		async (request, reply) => {
			...
			await db.query(
				"UPDATE `mensagens` SET `id` = ?, `autor` = ?, `valor` = ?, `hora` = NOW() WHERE `id` = ?;",
				[id, autor, valor, id],
			);
